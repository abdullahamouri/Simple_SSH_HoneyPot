...
def validate_dataframe(df, required_columns):
    if df is None or df.empty:
        print(f"[!] DataFrame is empty or None. Expected columns: {required_columns}")
        return False
    for column in required_columns:
        if column not in df.columns:
            print(f"[!] Missing column '{column}' in DataFrame. Columns available: {df.columns}")
            return False
    return True
...
if validate_dataframe(cmd_audits_log_df, ['Command']):
...
def country_lookup(country):
    if country == 'True':
        get_ip_to_country = ip_to_country_code(creds_audits_log_df)
        if validate_dataframe(get_ip_to_country, ['Country_Code']):
            top_country = top_10_calculator(get_ip_to_country, "Country_Code")
            return dbc.Col(dcc.Graph(figure=px.bar(top_country, x="Country_Code", y='count')), style={'width': '33%', 'display': 'inline-block'})
        else:
            return dbc.Col(html.Div("No country data available"), style={'width': '33%', 'display': 'inline-block'})
    else:
        return dbc.Col(html.Div("Country lookup disabled"), style={'width': '33%', 'display': 'inline-block'})
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(
            dcc.Graph(figure=px.bar(top_cmds, x='Command', y='count')) if validate_dataframe(top_cmds, ['Command', 'count']) 
            else html.Div("No command data available"), 
